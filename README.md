# 🚀 Linguagem e Técnicas de Programação 1 (LTP1)
...
| Semana | Período | Conteúdo Programático | Atividade Semanal |
| :---: | :---: | :--- | :--- |
| **7** | 04/08 a 08/08 | Introdução a APIs - O que é uma API? O padrão REST e o formato JSON. | Análise de uma API REST pública. |
| **8** | 11/08 a 15/08 | Introdução ao Django - Instalação e estrutura de um projeto. | Criação e entrega da estrutura de pastas do projeto Django. |
| **9** | 18/08 a 22/08 | DRF - Configuração inicial e criação do primeiro `Model`. | Codificar o arquivo `models.py` da aplicação. |
| **10** | 25/08 a 29/08 | DRF - Serializers: Convertendo `Models` em JSON. | Codificar o arquivo `serializers.py`. |
| **11** | 01/09 a 05/09 | DRF - Views e Endpoints: Criando a primeira rota para listar dados (GET). | Implementar a `APIView` para listagem de recursos. |
...
A avaliação da disciplina será baseada nas atividades práticas semanais e no projeto final.
